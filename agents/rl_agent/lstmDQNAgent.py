...
from rl_agent.model import DQNLSTM
...
class Agent: 
    def __init__(self, state_size, is_eval=False) :
        ...
        if os.path.exists('models_lstmDQN/target_model'):
            self.policy_net = torch.load('models_lstmDQN/policy_model', map_location=device)
            self.target_net = torch.load('models_lstmDQN/target_model', map_location=device)
        else:
            self.policy_net = DQNLSTM(state_size, self.action_size)
            self.target_net = DQNLSTM(state_size, self.action_size)
    ...
    def optimize(self):
        try:
            ...
            state_action_values = self.policy_net(state_batch).reshape((self.batch_size, 3)).gather(1, action_batch.reshape((self.batch_size, 1))) # PO0LICY
