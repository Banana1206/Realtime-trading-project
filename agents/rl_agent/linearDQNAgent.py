...
class Agent: 
    def __init__(self, state_size, is_eval=False) :
        ...
        if os.path.exists('models_linearDQN/target_model'):
            self.policy_net = torch.load('models_linearDQN/policy_model', map_location=device)
            self.target_net = torch.load('models_linearDQN/target_model', map_location=device)
        else:
